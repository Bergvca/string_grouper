...
* **`chunk_cols`**: The `sp_matmul_rs` counterpart to `n_blocks`: an `int` setting the column-chunk width of the backend's cache-blocked kernel.  This is a performance knob only — any value yields identical results.  It is mainly useful for tuning performance on matrices that are denser than those normally expected in the string-matching use case.  Defaults to `None`, which lets `sp_matmul_rs` derive the width from the detected L1d cache size.  Only used by the `sp_matmul_rs` backend (`use_sp_matmul_rs=True`); the `sparse_dot_topn` backend ignores it with a warning.
