...
class StringGrouper(object):
    ...
    def _symmetrize_matches_list(self):
        ...
        # swap columns and self-union
        self._matches_list = self._matches_list.merge(
            self._matches_list.rename(columns={'master_side': 'dupe_side', 'dupe_side': 'master_side'}),
            how='outer',
            on=['master_side', 'dupe_side']
        )
        self._matches_list['similarity'] = \
            self._matches_list[['similarity_x', 'similarity_y']].mean(skipna=True, axis=1)
        self._matches_list.drop(['similarity_x', 'similarity_y'], inplace=True, axis=1)
